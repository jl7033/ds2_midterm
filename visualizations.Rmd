...
wrap_plots(histograms) + plot_annotation(title = "Figure 3")
```
...
wrap_plots(boxplots) + plot_annotation(title = "Figure 4: Distribution of log-antibody levels by predictor")
```
...
wrap_plots(scatter_plots) + plot_annotation(title = "Figure 5: Relationship of log-antibody levels by predictor")
```
...
y = dat1$log_antibody
corrplot::corrplot(cor(x), method = "circle", type = "full", main = "Figure 6")
```
...
mars_fit = train(log_antibody ~ .,
               ...
               tuneGrid = mars_grid,
               trControl = ctrl1)
ggplot(mars_fit) + ggtitle("Figure 9")
```
#### Linear regression
```{r ols}
set.seed(2025)
...
bwplot(resamp, metric = "RMSE", main = "Figure 10: Comparing CV RMSEs") 
```